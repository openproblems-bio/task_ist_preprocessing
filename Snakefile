...
use rule annotate_counts_txsim_methods as annotate_counts with:
    wildcard_constraints:
        ct_method="tangram|pciSeqCT|mfishtools|FRmatch|tangram|NWCS"
    input:
        counts = '{results}/{dataset}/replicate{rep_id}/normcounts_{method}.h5ad',
        scd = '{results}/{dataset}/sc_normalized.h5ad',
...
def input_files_for_consensus_annotation(id_code, results, dataset, rep_id):
    ct_methods = parsed.get_method_params("NWCS", id_code).get("ct_methods")
...
rule annotate_celltypes_NWCS:
    ...
    output:
        '{results}/{dataset}/replicate{rep_id}/celltype_annotations_{method}_NWCS-{id_code}.csv'
    params:
        hyper_params = lambda w: get_params('NWCS', int(w.id_code), 'hyper_params'),
        group_params = lambda w: get_params('NWCS', int(w.id_code), 'group_params')
    shell:
