...
# Test run: all default methods + Cellpose 4 (cellposev4) segmentation, with a
# parameter sweep over the optimization levers identified for Cellpose-SAM.
# See src/methods_segmentation/cellposev4/NOTES.md ("Optimization / tuning").
#
# NOTE: cellposev4 (cpsam / SAM ViT) is GPU-heavy. Run on a CUDA-capable Docker
# host; on CPU it is very slow (the documented reason it is off the standard CI
# test path).
...
echo "Running benchmark on test data (defaults + cellposev4 sweep)"
echo "  Make sure to run 'scripts/project/build_all_docker_containers.sh'!"

# generate a unique id
RUN_ID="testrun_cellposev4_$(date +%Y-%m-%d_%H-%M-%S)"
...
gene_efficiency_correction_methods:
  - no_correction
method_parameters_yaml: $REPO_ROOT/scripts/run_benchmark/cellposev4_params.yaml
HERE
...
# The cellposev4 parameter sweep lives in a committed file (single source of truth,
# shared with run_test_cellposev4_nebius.sh): scripts/run_benchmark/cellposev4_params.yaml
# It defines a `default:` variant + one-arg-at-a-time `sweep:` variants (a "star"
# around the default, not a grid) => 19 cellposev4 variants. Edit that file to
