...
# Nebius test run: all default methods + watershed segmentation, with a parameter
# sweep over the Tier-1 optimization levers identified for classic marker-controlled
# watershed. See src/methods_segmentation/watershed/NOTES.md ("Optimization / tuning").
# Local sibling: run_test_watershed_local.sh
#
# watershed is a CPU method (midcpu/midmem/hightime) — NO gpu label, and it reuses
# the standard (non-GPU) Nebius test compute env.
#
...
#   file (scripts/run_benchmark/watershed_params.yaml) and read it from GitHub via
...
publish_dir="/scratch/results/runs/$(date +%Y-%m-%d_%H-%M-%S)_watershed"
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/watershed_params.yaml to '$params_branch' first." >&2
  exit 1
fi
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --labels task_ist_preprocessing,test,watershed
