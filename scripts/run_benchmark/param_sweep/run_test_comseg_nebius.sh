...
# Nebius test run: all default methods + comseg at the transcript-assignment stage,
# with a parameter sweep over comseg's tuning knobs.
# See src/methods_transcript_assignment/comseg/NOTES.md ("Optimization / tuning").
#
# The stage default (basic_transcript_assignment) stays enabled alongside comseg so the run
# also produces the baseline the sweep is scored against — the workflow allows at most ONE
# non-default variant at a time, and comseg is that one non-default method here; every OTHER
...
# scripts/run_benchmark/param_sweep/comseg_params.yaml to $params_branch before launching.
...
publish_dir="/scratch/results/runs/$(date +%Y-%m-%d_%H-%M-%S)_comseg"
...
params_url="https://raw.githubusercontent.com/${params_repo}/${params_branch}/scripts/run_benchmark/param_sweep/comseg_params.yaml"
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/param_sweep/comseg_params.yaml to '$params_branch' first." >&2
  exit 1
fi
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --labels task_ist_preprocessing,test,comseg
