...
#
# The stage default (basic_transcript_assignment) stays enabled alongside segger so the run
# also produces the baseline the sweep is scored against — the workflow allows at most ONE
# non-default variant at a time, and segger is that one non-default method here; every OTHER
...
# scripts/run_benchmark/param_sweep/segger_params.yaml to $params_branch before launching.
...
publish_dir="/scratch/results/runs/$(date +%Y-%m-%d_%H-%M-%S)_segger"
...
params_url="https://raw.githubusercontent.com/${params_repo}/${params_branch}/scripts/run_benchmark/param_sweep/segger_params.yaml"
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/param_sweep/segger_params.yaml to '$params_branch' first." >&2
  exit 1
fi
