...
# Nebius test run: all default methods + Cellpose 4 (cellposev4) segmentation,
# with a parameter sweep over the optimization levers identified for Cellpose-SAM.
# See src/methods_segmentation/cellposev4/NOTES.md ("Optimization / tuning").
# Local sibling: run_test_cellposev4_local.sh
#
# cellposev4 (cpsam / SAM ViT) is GPU-heavy, hence the `gpu` label and the Nebius
# GPU compute env.
#
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --labels task_ist_preprocessing,test,cellposev4,gpu
