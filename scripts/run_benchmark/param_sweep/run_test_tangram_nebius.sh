...
# Nebius test run: all default methods + Tangram (tangram) cell-type annotation,
# with a parameter sweep over the optimization levers identified for Tangram.
# See src/methods_cell_type_annotation/tangram/NOTES.md ("Optimization / tuning").
#
# Tangram is a torch deep-learning mapping method (GPU), hence the `gpu` label and
# the Nebius GPU compute env (mirrors run_gpu_nebius.sh).
#
# The annotation stage keeps its default method `tacco` alongside `tangram` so the
# run has a baseline to compare the tangram variants against. Every other stage is
...
#   (scripts/run_benchmark/param_sweep/tangram_params.yaml) and read it from GitHub
...
publish_dir="/scratch/results/runs/$(date +%Y-%m-%d_%H-%M-%S)_tangram"
...
params_url="https://raw.githubusercontent.com/${params_repo}/${params_branch}/scripts/run_benchmark/param_sweep/tangram_params.yaml"
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/param_sweep/tangram_params.yaml to '$params_branch' first." >&2
  exit 1
fi
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --labels task_ist_preprocessing,test,tangram,gpu
