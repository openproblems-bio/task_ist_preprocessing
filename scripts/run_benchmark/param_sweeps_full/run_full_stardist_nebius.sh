...
# Nebius FULL run: all default methods + StarDist2D (stardist) segmentation, driven by the
# FOLLOW-UP (narrowed, Pareto-selected) parameter sweep, on the FULL datasets and the FULL
# (non-test) labels_nebius.config.
#
# Sweep file: param_sweeps_full/stardist_params.yaml  (1 default + 7 swept = 8 variants;
#   AUTO-GENERATED from the test-resource sweep results — do not hand-edit it here).
# Small/test sibling: ../param_sweep/run_test_stardist_nebius.sh
#
# stardist runs on TensorFlow and is scheduled with the `gpu` label (GPU-capable base
# image); GPU routing is by src/base/labels_nebius.config nodeSelectors on that label.
#
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/param_sweeps_full/stardist_params.yaml to '$params_branch' first." >&2
  exit 1
fi
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --params-file /tmp/params_stardist_full.yaml \
  --entry-name auto \
  --config src/base/labels_nebius.config \
  --labels task_ist_preprocessing,full,stardist,gpu
