...
# Nebius FULL run: all default methods + watershed segmentation, driven by the FOLLOW-UP
# (narrowed, Pareto-selected) parameter sweep, on the FULL datasets and the FULL (non-test)
# labels_nebius.config.
#
# Sweep file: param_sweeps_full/watershed_params.yaml  (1 default + 9 swept = 10 variants;
#   AUTO-GENERATED from the test-resource sweep results — do not hand-edit it here).
# Small/test sibling: ../param_sweep/run_test_watershed_nebius.sh
#
# watershed is a CPU method (midcpu/midmem/hightime) — NO gpu label, and it uses the same
# Nebius compute env; resources come from src/base/labels_nebius.config.
#
...
if ! curl -fsSL -o /dev/null "$params_url"; then
  ...
  echo "  $params_url" >&2
  echo "Commit and push scripts/run_benchmark/param_sweeps_full/watershed_params.yaml to '$params_branch' first." >&2
  exit 1
fi
...
tw launch https://github.com/openproblems-bio/task_ist_preprocessing.git \
  ...
  --params-file /tmp/params_watershed_full.yaml \
  --entry-name auto \
  --config src/base/labels_nebius.config \
  --labels task_ist_preprocessing,full,watershed
