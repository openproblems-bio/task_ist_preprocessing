...
if __name__ == '__main__':
    ...
    for replicate in replicate_folders:
        metric_file_name = os.path.join(replicate, f"group_metrics-{id_code}.csv")
    ...
    aggregated_metric = pd.read_csv(os.path.join(data, f"aggregated/group_metrics-{id_code}.csv"), index_col=[0,1])
    
    metric = tx.metrics.aggregate_group_metrics(metric_list, aggregated_metric, name_list=name_list)
    metric.to_csv(os.path.join(data, f"aggregated/aggregated_group_metrics-{id_code}.csv"))
