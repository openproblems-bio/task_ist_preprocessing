// pciseq_transcript_assignment build_main
// 
...
meta = [
  ...
  "argument_groups" : [
    {
      ...
      "arguments" : [
        ...
        },
        {
          "type" : "double",
          "name" : "--MisreadDensity",
          "description" : "\\"To account for spots far from the some a uniform distribution is introduced to describe those misreads.\nBy default this uniform distribution has a density of 1e-5 misreads per pixel.\\"\n",
          "default" : [
            1.0E-5
          ],
          "required" : false,
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_ist_preprocessing/task_ist_preprocessing/src/methods_transcript_assignment/pciSeq_transcript_assignment/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker|native",
    "output" : "target/nextflow/methods_transcript_assignment/pciseq_transcript_assignment",
    "viash_version" : "0.9.4",
...
par = {
  ...
  'CellCallTolerance': $( if [ ! -z ${VIASH_PAR_CELLCALLTOLERANCE+x} ]; then echo "float(r'${VIASH_PAR_CELLCALLTOLERANCE//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'rGene': $( if [ ! -z ${VIASH_PAR_RGENE+x} ]; then echo "float(r'${VIASH_PAR_RGENE//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'Inefficiency': $( if [ ! -z ${VIASH_PAR_INEFFICIENCY+x} ]; then echo "float(r'${VIASH_PAR_INEFFICIENCY//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'InsideCellBonus': $( if [ ! -z ${VIASH_PAR_INSIDECELLBONUS+x} ]; then echo "float(r'${VIASH_PAR_INSIDECELLBONUS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'MisreadDensity': $( if [ ! -z ${VIASH_PAR_MISREADDENSITY+x} ]; then echo "float(r'${VIASH_PAR_MISREADDENSITY//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'SpotReg': $( if [ ! -z ${VIASH_PAR_SPOTREG+x} ]; then echo "float(r'${VIASH_PAR_SPOTREG//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'nNeighbors': $( if [ ! -z ${VIASH_PAR_NNEIGHBORS+x} ]; then echo "int(r'${VIASH_PAR_NNEIGHBORS//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'rSpot': $( if [ ! -z ${VIASH_PAR_RSPOT+x} ]; then echo "float(r'${VIASH_PAR_RSPOT//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'save_data': $( if [ ! -z ${VIASH_PAR_SAVE_DATA+x} ]; then echo "r'${VIASH_PAR_SAVE_DATA//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi )
}
meta = {
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_ist_preprocessing/methods_transcript_assignment/pciseq_transcript_assignment",
    "tag" : "build_main"
  },
  "label" : [
    "midtime",
