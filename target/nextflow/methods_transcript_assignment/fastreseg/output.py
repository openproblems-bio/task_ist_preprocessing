...
sdata_transcripts_only = sd.SpatialData(
    ...
    tables={
        "table": ad.AnnData(
          # The transcript-assignment output format requires a `cell_id` column in
          # the table's obs (see src/api/file_transcript_assignments.yaml); expose
          # fastReseg's updated segmentation under the standard column names.
          obs=cell_df.loc[:,['updated_cellID', 'updated_celltype']].rename(
            columns={'updated_cellID': 'cell_id', 'updated_celltype': 'cell_type'}
          ),
