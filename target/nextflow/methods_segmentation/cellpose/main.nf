...
meta = [
  ...
  "build_info" : {
    "config" : "/home/runner/work/task_ist_preprocessing/task_ist_preprocessing/src/methods_segmentation/cellpose/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker|native",
    "output" : "target/nextflow/methods_segmentation/cellpose",
    "viash_version" : "0.9.0",
    "git_commit" : "9e32a2d35c1b2920e3be6f749db97babd692fb65",
...
par = {
  ...
  'model_type': $( if [ ! -z ${VIASH_PAR_MODEL_TYPE+x} ]; then echo "r'${VIASH_PAR_MODEL_TYPE//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'channel_axis': $( if [ ! -z ${VIASH_PAR_CHANNEL_AXIS+x} ]; then echo "r'${VIASH_PAR_CHANNEL_AXIS//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'z_axis': $( if [ ! -z ${VIASH_PAR_Z_AXIS+x} ]; then echo "r'${VIASH_PAR_Z_AXIS//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'normalize': $( if [ ! -z ${VIASH_PAR_NORMALIZE+x} ]; then echo "r'${VIASH_PAR_NORMALIZE//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
  'invert': $( if [ ! -z ${VIASH_PAR_INVERT+x} ]; then echo "r'${VIASH_PAR_INVERT//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
  'rescale': $( if [ ! -z ${VIASH_PAR_RESCALE+x} ]; then echo "r'${VIASH_PAR_RESCALE//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'diameter': $( if [ ! -z ${VIASH_PAR_DIAMETER+x} ]; then echo "float(r'${VIASH_PAR_DIAMETER//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'do_3D': $( if [ ! -z ${VIASH_PAR_DO_3D+x} ]; then echo "r'${VIASH_PAR_DO_3D//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
  'anisotropy': $( if [ ! -z ${VIASH_PAR_ANISOTROPY+x} ]; then echo "r'${VIASH_PAR_ANISOTROPY//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'augment': $( if [ ! -z ${VIASH_PAR_AUGMENT+x} ]; then echo "r'${VIASH_PAR_AUGMENT//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
  'tile_overlap': $( if [ ! -z ${VIASH_PAR_TILE_OVERLAP+x} ]; then echo "float(r'${VIASH_PAR_TILE_OVERLAP//\\'/\\'\\"\\'\\"r\\'}')"; else echo None; fi ),
  'resample': $( if [ ! -z ${VIASH_PAR_RESAMPLE+x} ]; then echo "r'${VIASH_PAR_RESAMPLE//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
  'interp': $( if [ ! -z ${VIASH_PAR_INTERP+x} ]; then echo "r'${VIASH_PAR_INTERP//\\'/\\'\\"\\'\\"r\\'}'.lower() == 'true'"; else echo None; fi ),
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "openproblems-bio/task_ist_preprocessing/methods_segmentation/cellpose",
    "tag" : "build_main"
  },
  "label" : [
    "midtime",
